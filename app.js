...
function cipher(string){
	...
	for (var i = 0 ; i < string.length ; i++){ 
		if (string[i] == " "){ 
			...
			character = (((character - 97) + 33) % 26) + 97; // El 97 representa el inicio del alfabeto en minúsculas en ASCII
			character = String.fromCharCode(character);
			newString = newString + character;
		}else{
			var character = string.charCodeAt(i); 
			character = (((character - 65) + 33) % 26) + 65;  // El 65 representa el inicio del alfabeto en mayúsculas en ASCII
			character = String.fromCharCode(character); 
			newString = newString + character; 
		}
	}
	document.write("<center>En caso de que quieras copiar tu texto, aquí lo tienes:<br><br>" + "\"" + newString + "\"</center>");
	return alert("Tu texto cifrado se ve así: \n" + newString); //document.write primero para que el return no corte el proceso.
}
...
function decipher(string){
	...
	for (var i = 0 ; i < string.length ; i++){ 
		if (string[i] == " "){ 
			...
			var character = string.charCodeAt(i);
			character = ((((character - 97) - 7) + 26) % 26) + 97; /* Reemplazamos el 33 por 7 porque es equivalente 
